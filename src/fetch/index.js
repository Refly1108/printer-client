...
export const getPrintListFromServer = async (printerId) => {
  let opts = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
  ...
  let result = await getPrintListValidation(
    await fetchRequest(config.url.getWish+"?printerId="+printerId, opts, true)
  );
  return result;
};
...
export const finishWishJobToServer = async (job) => {
  ...
  let result = await getPrintListValidation(
    await fetchRequest(config.url.finishWish+"wishId="+job.wishId+"&printerId="+job.printerId, opts, false)
  );
  return result;
};
